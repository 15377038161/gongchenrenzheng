...
│   └── vite.ts           # Vite 集成逻辑
├── src/                   # 前端源码目录
│   ├── index.tsx         # 前端应用入口（createRoot 渲染）
│   ├── main.tsx          # 前端主逻辑文件（React 组件）
...
- **生产模式** (`coder start`)：
  - `coder build` 构建前端 → `dist/` 目录
  - `coder build` 构建后端 → `dist-server/index.js` (CommonJS 格式)
...
1. 运行 `coder build` 构建前后端
2. 将整个项目上传到服务器
3. 运行 `pnpm install --prod`
4. 运行 `coder start` 启动服务
